...
static char _direct[8] = {0}; // Stores "in" or "out"
static bool allow_led_control = false; // Flag to control LED
...
static ssize_t direction_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count) {
    if (strncmp(buf, "in", 2) == 0) {
        strcpy(_direct, "in");
        allow_led_control = false; // Disable LED control when direction is "in"
    } else if (strncmp(buf, "out", 3) == 0) {
        strcpy(_direct, "out");
        allow_led_control = true; // Enable LED control when direction is "out"
    } else {
        return count; // Ignore any other values
    }
